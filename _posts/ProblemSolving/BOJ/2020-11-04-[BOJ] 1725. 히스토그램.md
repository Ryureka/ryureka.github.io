...
마찬가지로 알고스팟(Algospot)의 [울타리 잘라내기](https://algospot.com/judge/problem/read/FENCE)와 릿코드(leetcode)의 [largest-rectangle-in-histogram](https://leetcode.com/problems/largest-rectangle-in-histogram/)에서도 풀어보실 수 있습니다.
...
public class Main {
	...
	public static long go(int left,int right) {
		...
		long left_part = go(left,mid);
		long right_part = go(mid,right);
		int min = Math.min(arr[mid - 1],arr[mid]);
		long middle_part = min*2;
		int right_cnt = 1;
		int left_cnt = 1;
		long bigger_part = 0;
		while(left_cnt + right_cnt != right - left) {
			if(mid - 1 - left_cnt < left) {
				min = Math.min(min,arr[mid + right_cnt]);
				bigger_part = Math.max(bigger_part, (long)(min)*(1 + left_cnt + right_cnt));
				right_cnt++;
			}
			else if(mid + right_cnt > right-1) {
				min = Math.min(min,arr[mid - 1 - left_cnt]);
				bigger_part = Math.max(bigger_part, (long)(min)*(1 + left_cnt + right_cnt));
				left_cnt++;
			}
			else {
				if(arr[mid - 1 - left_cnt] > arr[mid + right_cnt]) {
					min = Math.min(min,arr[mid - 1 - left_cnt]);
					bigger_part = Math.max(bigger_part, (long)(min)*(1 + left_cnt + right_cnt));
					left_cnt++;
				}else {
		...
		middle_part = Math.max(middle_part, bigger_part);
		return Math.max(middle_part, Math.max(left_part,right_part)); 
	}
}
~~~
<!-- for문이 2번 중첩되어 \\(O(N^2)\\)의 시간복잡도를 가지게 됩니다. 하지만 마찬가지로 히스토그램 문제의 입력 N의 최대값은 \\(100,000(=10^5)\\) 이므로 최대 \\(10^{10}\\)정도의 시간이 걸리게 된다고 생각해 볼 수 있습니다. 현대 컴퓨터에서의 수행시간을 대략 반복문 수행횟수 1억(=\\(10^8\\)) 당 1초가 걸린다고 가정한다면 약 100초(=\\(10^2\\)) 가까이 걸리게 되므로 문제에 명시된 2초의 제한시간으로는 시간초과가 발생함을 예상해 볼 수 있습니다. 116일에서 100초로 어느정도 성능이 향상되었지만 아직 제한시간에는 못 미치기에 다른 방법을 생각해내야합니다. -->
